...
namespace ISDCompanion.Services.InfoTextServices
{
    internal class PageReplacement_LRU_InfoTextService : IInfoTextService
    {
        ...
        public PageReplacement_LRU_InfoTextService(List<IPageReplacementStep> steps)
        {
            _steps = steps;
        }
        ...
        public string[] GetInfoTexts()
        {
            ...
            foreach (IPageReplacementStep step in _steps)
            {
                ...
                else
                {
                    ...
                    //Kacheln waren voll, nach LRU Algorithmus wird die Kachel ersetzt,
                    //wessen nächste Nutzung am nähesten in der Vergangenheit liegt
                    else
                    {
                        if (distance > 1000)
                        {
                        ...
                        {
                            infoText = String.Format(AppResources.InfoText_PageReplacement_LRU_Element_Added_Distance, distance);
                        }
                    }
                }
